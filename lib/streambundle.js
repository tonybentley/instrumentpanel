  ...
  if(delta.updates) {
    delta.updates.forEach(function(update) {
      ...
      update.values.forEach(function(pathValue) {
        if(pathValue.path) {
          pathValue.timestamp = update.timestamp
...
StreamBundle.prototype.push = function(sourceId, pathValue) {
  ...
  let bus = this.getBusForSourcePath(sourceId, pathValue.path)
  const value = pathValue.value;

  //TODO: Figure out why this is getting converted
  const timeValue = new Date(pathValue.timestamp).getTime();

  console.log(pathValue.path, timeValue, typeof timeValue)

  bus.push([value, timeValue])
  
  this.allSources.push({
    sourceId: sourceId,
    path: pathValue.path,
    key: keyForSourceIdPath(sourceId, pathValue.path),
