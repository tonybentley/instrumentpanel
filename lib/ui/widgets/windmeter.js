...
class ComponentClass extends React.Component {
  ...
  render() {
    ...
    return (
      <svg onClick={this.changeDisplayMode} height="100%" width="100%" viewBox="0 0 400 400">
        ...
        <text x="200" y="150" textAnchor="middle" fontSize="16">
  ...
  componentDidMount() {
    ...
    that.props.timestamp.onValue(function(timestamp){
      //TODO: the value comes from an active source, but will display in an inactive source
      if(that._intervals[that.props.options.path]){
        clearInterval(that._intervals[that.props.options.path])
      }
      that._intervals[that.props.options.path] = setInterval(
        function(){
          if(that._isMounted){
            that.setState({
...
function WindMeter(id, options, streamBundle, instrumentPanel) {
  ...
  this.widget = React.createElement(ComponentClass,{
    ...
    timestamp: streamBundle.getStreamForSourcePath(options.sourceId, options.path +'.'+ options.sourceId),
    values: values,
    activeStreams: {},
    holder: {
      setState: function() {}
