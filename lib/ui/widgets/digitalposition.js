  ...
  class ReactComponent extends React.Component {
    ...
    componentDidMount() {
      let that = this;
        ...
        streamBundle.getStreamForSourcePath(options.sourceId, options.path+'.'+options.sourceId).onValue(function(timestamp) {
           if(that._intervals[options.path]){
              clearInterval(that._intervals[options.path])
            }
            that._intervals[options.path] = setInterval(
              function(){
                if(that._isMounted){
                  that.setState({
       ...
       if (that.unsubscribe) {
        ...
        that.unsubscribe = streamBundle.getStreamForSourcePath(options.sourceId, options.path).onValue(function(value) {
          if(that._isMounted){
            that.setState(value);
          }
        });
