...
var labelForPath = {
  'navigation.courseOverGroundTrue' : 'Course Over Ground True',
  'navigation.courseOverGroundMagnetic' : 'Course Over Ground Magnetic',
  ...
  'environment.water.temperature': 'Temperature Below Hull',
  ...
  'environment.wind.directionMagnetic' : 'Wind Direction Magnetic',
  'environment.wind.directionTrue' : 'True Wind Direction',
  'propulsion.0.oilPressure' : 'Engine Oil Pressure',
  'propulsion.0.coolantTemperature' : 'Engine Coolant Temperature'
};
...
BaseWidget.prototype.updateStream = function(widget, valueStream, timestampStream) {
...
BaseWidget.prototype.updateUnitData = function(widget) {
  ...
  .then(meta => {
    ...
    var timestampStream = widget.streamBundle.getStreamForSourcePath(widget.options.sourceId, widget.options.path + '.' + widget.options.sourceId); 
    ...
    widget.updateStream(widget, valueStream, timestampStream);
  })
}
