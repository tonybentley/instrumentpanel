...
export default class CompassRoseComponent extends React.Component {
  ...
  render() {
    return (
      ...
      <g transform="scale(0.8) translate(50 50)">
        ...
        <text x="200" y="300" fontSize="12" textAnchor="middle">
  ...
  componentWillReceiveProps(nextProps) {
    var that = this;
    if(nextProps.timestamp !== that.props.timestamp){
      nextProps.timestamp.onValue(function(timestamp) {
    ...
    if(nextProps.value !== that.props.value) {
      if (nextProps.value) {
        ...
        that.unsubscribe = nextProps.value.onValue(function(value) {
          that.setState({
            value: value
          });
        }.bind(that));
  ...
  componentDidMount() {
    ...
    if (that.props.value) {
      ...
      that.unsubscribe = that.props.value.onValue(function(value) {
        that.setState({
          value: value,
        });
      });
